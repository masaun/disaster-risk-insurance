const HoneycombBetPool = artifacts.require("HoneycombBetPool");
...
module.exports = async function (deployer) {
    await deployer.deploy(HoneycombBetPool, linkTokenAddress, oracle, jobId, paymentAmount);
    const honeycombBetPool = await HoneycombBetPool.deployed();

    const linkToken = await LinkTokenInterface.at(linkTokenAddress);
    await linkToken.transfer(honeycombBetPool.address, paymentAmount);
};